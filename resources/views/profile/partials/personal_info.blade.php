<div class="card m-5 ">
    ...
    <div class="card-body">
        ...
        <form name="profile" action="{{route('update_profile')}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row justify-content-between">
                <div class="col-md-3 text-center">
                    <div class="image-input image-input-outline mask image-input-circle  image-input-empty" data-kt-image-input="true" style="background-image: url('/metronic8/demo1/assets/media/svg/avatars/blank.svg')">
                        <div class="image-input-wrapper  w-150px h-150px" style="background-image:url('{{asset($profile_info["img_profile"] )}}');background-color: rgba(0, 0, 0, 0.5);background-blend-mode: overlay;"></div>
                ...
                <div class="col-md-8">
                    <div class="row">
                        <div class="col-md-6 my-2">
                            <label class="style-label-form">{{cp('full-name')}}</label>
                            <input type="text" name="first_name" class="form-control"
                                   value="{{$profile_info['first_name']}}"/>
                        </div>
                        <div class="col-md-6 my-2">
                            <label class="style-label-form">{{cp('phone-number')}}</label>
                            <input type="text" name="phone" class="form-control"
                                   value="{{auth('customers')->user()->phone}}"/>
                        </div>
                        <div class="col-md-6 my-2">
                            <label class="style-label-form">{{cp('email')}}</label>
                            <input type="email" class="form-control" disabled
                                   value="{{auth('customers')->user()->email}}"/>
                        </div>
                        <div class="col-md-6 my-2">
                            ...
                            <select name="country_code" data-control="select2"
                                ...
                                @foreach($collected_data['countries'] as $country)
                                    <option value="{{$country->id}}" @if($country->id == auth('customers')->user()->country->id) selected @endif>{{$country->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 my-2">
                            <label class="style-label-form">{{cp('facebook-account')}}</label>
                            <input type="text" name="facebook_acc" class="form-control"
                                   value="{{auth('customers')->user()->facebook_acc}}"/>
                        </div>
                        <div class="col-md-6 my-2">
                            <label class="style-label-form">{{cp('whatsapp-number')}}</label>
                            <input type="text" name="whatsapp_acc" class="form-control"
                                   value="{{auth('customers')->user()->whatsapp_acc}}"/>
                        </div>

                        <div class="col-md-6 my-2">
                            {{--                            <label class="style-label-form">{{cp('whatsapp-number')}}</label>--}}
                            <input type="submit" class="form-control BntAdd-Modal"
                                   style="color: white !important;" value="{{trans('lang.save')}}"/>
                        </div>
                    </div>
                </div>
