            ...
            <div class="page-title style-boder-titel-card d-flex flex-column   ">
                <h1 class="style-title-card px-4 py-4">
                                                <span class="fw-bolder mb-2 text-dark">
                                                    {{cp('protection_and_security_title')}}
                                                </span>
                </h1>
            ...
            <div class="row">
                    ...
                    <div class="col-md-5">
                        <div class="fv-row mb-10">
                            ...
                            <select name="confirmation_notification" id="confirmation_notification"
                                    ...
                                    data-placeholder="{{cp('successful_license_notification_placeholder')}}"
                                    data-hide-search="true"
                                    class="form-select style-select-profile style-label-form  ">
                                {{--                                <option value="1">إرسال عند تغيير الشبكة الفرعية</option>--}}
                                <option value="0" @if(auth()->user()->confirmation_notification == 0) selected @endif>{{cp('never_send_a_verification_code')}}</option>
                                <option value="2" @if(auth()->user()->confirmation_notification == 2) selected @endif>{{cp('ip_send_when_address_change')}}</option>
                                <option value="1" @if(auth()->user()->confirmation_notification == 1) selected @endif>{{cp('always_send_code')}}</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="col-md-5">
                        <div class="fv-row mb-10">
                            <label class="fs-6 fw-bold form-label style-label-form mb-2"
                                   style="color:#1B3160;">{{cp('confirmation_method')}}</label>
                            <select name="confirmation_method" id="confirmation_method" data-control="select2"
                                    data-placeholder="{{cp('confirmation_method_placeholder')}}"
                                    data-hide-search="true"
                                    class="form-select style-select-profile style-label-form  ">
                                <option value="0" @if(auth()->user()->confirmation_method == 0) selected @endif>{{cp('e-mail')}}</option>
{{--                                <option value="1" @if(auth()->user()->confirmation_method == 1) selected @endif>{{cp('message_sms')}}*</option>--}}
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-5">
                        <div class="fv-row mb-10 mt-2">
                            <button type="button" id="protectionSecurityBtn"
                                    {{--                                    onclick="ClicksuccesInputCodeConifremInfo()"--}}
                                    {{--                                    data-bs-toggle="modal" data-bs-target="#kt_modal_master_key"--}}
                                    class="form-control BntAdd-Modal">{{cp('confirm')}}
                            </button>
{{--                            <label class="pt-3 style-title-bio-card2 ">--}}
{{--                                <span style="color:#E51C39!important;">*</span>--}}
{{--                                {{cp('the_cost_of_one_message')}} $0.05--}}
{{--                            </label>--}}
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-header">
                ...
                <div>
                    <h2 class="fw-bolder style-Address-Modal">
                        ...
                        <span class="px-1">{{cp('master_key')}}</span>
                    </h2>
                    <h2 class="pt-1">
                        <span class="style-bio-Modal-trasfer">{{cp('please_enter_3_numbers')}}</span>
                    </h2>
                </div>
                            ...
                            <div class="d-flex  flex-row mw-md-600px w-100">
                                ...
                                <span class="px-3">
                                                        <input type="number" readonly style="width:185px;" class="form-control"
                                                               placeholder="XXX" id="masterKeysInput" value="{{auth()->user()->is_master_key_enabled == 1 ? auth()->user()->master_key : ''}}"/>
                                                    </span>
                            </div>
...
<script>
    $(document).ready(function () {
        ...
        function sendProtectionSecurityRequest() {
            ...
            $.ajax({
                url: '{{route('update_protection_and_security')}}',
                type: "post",
                data: {
                    _token: '{{csrf_token()}}',
