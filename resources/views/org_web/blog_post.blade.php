...
@section('keywords')
    <meta name="keywords" content="{{ isset($post->keywords)?$post->keywords:'ytifs' }}" />
@endsection
        ...
        <div class="container">
            <section class="post-info">
                ...
                <section class="post-details">
                    <h1>{{isset($post->post_title)?$post->post_title:''}}</h1>
                    <h5>{{$post->post_subtitle}}</h5>
                    <div class="post-user-time">
                        <div class="user">
                            <img src="{{asset('/org_assets/dist/img/user-placeholder.jpg')}}" alt="">
                            <h6>{{isset($post->author_post)?$post->users->name:''}}</h6>
                        </div>
                        <div class="time">
                            <i class="fas fa-clock"></i>
                            <span>
                                <span>   <?php echo \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() ?> </span> -
                                <span>
                                  <?php echo \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->toDateString() ?>
                                </span>
                            </span>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <section class="post-likes">
                        ...
                        <span class="no">{{isset($post->views)?$post->views:''}}</span>
                ...
                <div class="col-sm-12">
                    <div class="share-links">
                        ...
                        <a href="http://twitter.com/share?text={{ $post->post_title }}&url={{ Request::fullUrl() }}&hashtags={{ $post->keywords }}" class="item twitter">
                            <i class="fab fa-twitter"></i>
                            <span>{{ __('site.twitt') }}</span>
                        </a>
                        <a href="https://api.whatsapp.com/send?text={{$post->post_title}}" class="item whatsapp" target="_blank">
                            <i class="fab fa-whatsapp"></i>
                            <span>{{ __('site.whatsApp') }}</span>
                        </a>
                        <a href="https://telegram.me/share/url?url={{ Request::fullUrl() }}&text={{ $post->post_title }}" target="_blank" class="item telegram">
                    ...
                    <section class="post-footer">
                        <section class="post-shortlink">
                            <span>
                                {{ __('site.short_link') }}
                                : </span>
                            <span class="url"><a href="{{ $post->short_link }}">
                                    {{ $post->short_link }}
                                </a></span>
                       ...
                       {{--     <section class="rate-author">
                                ...
                                <div class="rate">
                                    ...
                                    <input type="hidden" name="writer_id" value="{{isset($post->author_post)?$post->author_post:0}}">
                                </div>
                       ...
                       {{--     <section class="post-reactions">
                                ...
                                <form class="emojis" id="emoji" method="post">
                                    ...
                                    <input type="hidden" name="review_for" value="posts">
            ...
            <div class="row post-comment-section">
                <section class="col-sm-12">
                    <form class="new-comment" action="#" method="post">
                        ...
                        <input type="hidden" name="post_id" value="{{ $post->id }}">
                        <input type="hidden" name="comment_for" value="posts">
