    ...
    <section class="m-5 ">
        ...
        <div class="row justify-content-row-currency">
            <div class="col-md-11 style-card-currencey-form style-right mx-3 my-3">
                <div class="card">
                    <div class="card-body">
                        <form action="{{route('wallet.convert_currency')}}" method="POST">
                            @csrf
                            <div class="row justify-content-en">

                                <div class="col-md-3 style-currency-trasnform">
                                    <label class="style-label-form">{{cp('deposit_amount')}}</label>
                                    <input type="text" class="form-control" id="amount" name="amount" value="@if(isset($amount)) {{$amount}} @endif" required/>
                                </div>
                                <div class="col-md-3  style-currency-trasnform3">
                                    <label class="style-label-form">{{cp('from_amount')}}</label>
                                    <select class="form-select" data-placeholder="{{cp('select_curr')}}"
                                            id="kt_docs_select2_country_Currency_exchange_Form" name="exchange_form">
                                        <option></option>
                                        @foreach($currencies as $currency)
                                            <option data-kt-select2-country="{{asset($currency->img_path)}}" value="{{$currency->code}}"
                                                    @if(isset($exchange_form) && $exchange_form == $currency->code) selected @endif
                                            >{{$currency->name}}</option>
                                        @endforeach
                                        {{--                                        <option data-kt-select2-country="flags/united-states.svg">الدولار</option>--}}
                                        {{--                                        <option data-kt-select2-country="flags/european-union.svg">اليورو</option>--}}
                                        {{--                                        <option data-kt-select2-country="flags/egypt.svg">الجنية المصري</option>--}}
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3 style-currency-trasnform2">
                                    <label class="style-label-form">{{cp('to_amount')}}</label>
                                    <select class="form-select" data-placeholder="{{cp('select_curr')}}"
                                            id="kt_docs_select2_country_Currency_exchange_To" name="exchange_to">
                                        <option></option>
                                        @foreach($currencies as $currency)
                                            <option data-kt-select2-country="{{asset($currency->img_path)}}" value="{{$currency->code}}"
                                                    @if(isset($exchange_to) && $exchange_to == $currency->code) selected @endif
                                            >{{$currency->name}}</option>
                                        @endforeach
                                        {{--                                        <option data-kt-select2-country="flags/united-states.svg">الدولار</option>--}}
                                        {{--                                        <option data-kt-select2-country="flags/european-union.svg">اليورو</option>--}}
                        ...
                        <div class="style-card-body-currency">
                            <div class="d-flex style-right justify-content-between flex-row">
                                <div class="text-center pt-3">
                                    <span class="style-titel-currencys-cards-all">{{number_format($exchange_price, 4)}} {{$to_curr->symbol}}</span>
                                </div>
                                ...
                                <div class="pt-3">
                                    {{--                                    <img src="/assets_v2/media/imagesWebsite/Rectangle77.png"/>--}}
                                    <img src="{{$to_curr->img_path}}" alt="{{$to_curr->code}}" width="29px"/>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-3 my-3 style-card-features-respinsive">
                        <div class="card">
                            <div class="style-card-body-currency">
                                <div class="d-flex style-right justify-content-between flex-row">
                                    <div class="text-center pt-3">
                                        <span class="style-titel-currencys-cards-all">{{number_format($currency->exchange_price, 4)}}$</span>
                                    </div>
                                    <div class="text-center">
                                            <span class="style-titel-currencys-cards-all">
                                                {{$currency->code}}<br/>
                                    ...
                                    <div class="pt-3">
                                        {{--                                    <img src="/assets_v2/media/imagesWebsite/Rectangle77.png"/>--}}
                                        <img src="{{$currency->img_path}}" alt="{{$currency->code}}" width="29px"/>
                                    </div>
                                </div>
                            </div>
                        </div>
