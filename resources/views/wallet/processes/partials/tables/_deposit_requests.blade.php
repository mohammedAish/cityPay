@foreach($deposit_orders as $order)
    <tr class="odd  style-row-table">
        ...
        <td class="style-text-row-table">{{$order->agency->name}}</td>


        <td class="style-text-row-table">${{$order->amount}}</td>
        <td class="style-text-row-table">{{$order->deposit_type}}</td>
        <td class="style-text-row-table">
            <button type="button" class="btn show-deposit-details-modal" 
                    order_agency="{{$order->agency->name}}"
                    order_method="{{$order->agency->depositMethod->name}}"
                    order_amount="{{$order->amount}}"
                    order_id="{{$order->id}}"
                    order_currency="{{$order->currency->name}}"
                    order_total_amount="{{$order->final_amount}}"
                    order_commission="{{$order->agency->fixed_charge_deposit + $order->agency->deposit_fee_percent}}"
            >
                {{$order->id}}
            </button>
        </td>
        <td class="style-text-row-table">
            @if($order->current_status == 'pending')
                ...
                {{--                <span class="px-1">{{cp('pending')}}</span>--}}

                <span class="px-1">
                    <img id="img_path_{{$order->id}}" src="{{asset($order->img_path)}}" alt=""
                         style="max-width: 40px;">
                    <a href="#" data-order_id="{{$order->id}}" class="show_add_file_model"> 
                        {{cp('pending')}}{{'- ايصال الدفع'}}</a></span>

            @elseif($order->current_status == 'rejected')
                <svg width="15" height="15" viewBox="0 0 15 15" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
                    <circle cx="7.5" cy="7.5" r="6.5" stroke="#E51C39" stroke-width="1.5"/>
                    <path d="M7.86089 4.25003C7.86089 4.44946 7.69922 4.61114 7.49978 4.61114C7.30035 4.61114 7.13867 4.44946 7.13867 4.25003C7.13867 4.05059 7.30035 3.88892 7.49978 3.88892C7.69922 3.88892 7.86089 4.05059 7.86089 4.25003Z"
                ...
                <span class="px-1">{{cp('rejected')}}</span>
            @else
                <svg width="15" height="15" viewBox="0 0 15 15" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
