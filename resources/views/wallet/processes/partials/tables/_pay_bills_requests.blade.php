@foreach($pay_bills_orders as $order)
    <tr class="odd  style-row-table">
        <td class="style-text-row-table">{{$order->created_at->format('Y/m/d')}}</td>
        <td class="style-text-row-table">${{$order->real_price}}</td>


        <td class="style-text-row-table">${{$order->commission_fee}}</td>
        <td class="style-text-row-table">${{$order->final_price}}</td>
        ...
        <td class="style-text-row-table">
            ...
            @endif
            <span class="px-1">{{$order->current_status == 'pending' ? cp('pending') : cp('completed')}}</span>
        </td>
    </tr>
@endforeach    