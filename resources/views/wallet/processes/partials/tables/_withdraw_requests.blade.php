    ...
    <tr class="odd  style-row-table">
        ...
        <td class="style-text-row-table">
            <button type="button" class="btn show-withdraw-details-modal"
                    order_account_name="{{$order->agency->customer_agency_acc_name}}"
                    order_account_number="{{$order->agency->customer_agency_acc_number}}"
                    order_agency="{{$order->agency->name}}"
                    order_amount="{{$order->amount}}"
                    order_currency="{{$order->currency->name}}"
                    order_total_amount="{{$order->final_amount}}"
                    order_commission="{{$order->fee_amount}}"
                    all_data="{{$order}}"
            >
                {{$order->id}}
            </button>
        ...
        <td class="style-text-row-table">
            ...
            @endif
            <span class="px-1">{{cp($order->current_status)}}</span>
        </td>
    </tr>
@endforeach    