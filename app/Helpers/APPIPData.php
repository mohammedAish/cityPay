...
class APPIPData
{
    ...
    private static function getInstanceDevelopment() {
        $robj               = new self();
        $robj->ip           = "175.110.42.86";
        $robj->country_code = "YE";
        $robj->country_name = "Yemen";
        $robj->region_code  = "";
        $robj->region_name  = "";
        $robj->city         = "";
        $robj->zip_code     = "";
        $robj->time_zone    = "Asia/Aden";
        $robj->latitude     = 15.3522;
        $robj->longitude    = 44.2095;
    ...
    static function get($ip = "") {
        if (empty($ip)) {
            ...
            if (App::environment(['local', 'development'])
                && ($ip == "127.0.0.1" || $ip == "192.168.1.100" || $ip == "::1"
                                       || $ip == "192.168.10.71" || $ip == "192.168.10.1")) {
