...
function registerReferrer($referrer_id){
        $uniq = ['session_id' => session()->getId(),
                 'referrer_id' => $referrer_id, 'ip_address' => request()->ip()];
        Referrer::updateOrCreate($uniq);
}
/**
    ...
    if (!empty($valueParts)) {
        ...
        foreach ($valueParts as $valuePart) {
            if ($escapedChars <= 0) {
                ...
                $valuePart       = mb_substr($valuePart,0,$escapedChars).$hiddenSubString.mb_substr($valuePart,
                        -$escapedChars);
            }
 ...
 */
function t($string,$replace = [],$file = 'lang',$locale = null){
    if (is_null($locale)) {
...
/**
 * Default translator (e.g. en/global.php)
 * this is for my strings to find them later and translate.
 *
 * @param $string
 * @param  array  $replace
 * @param  string  $file
 ...
 * @return string
 */
function tt($string,$replace = [],$file = 'global',$locale = null){
    return $string;
}
...
function isFromRestApi(){
    $isFromApi = false;
    if (
        request()->segment(1) == 'api' || request()->segment(2) == 'api'
    ) {
...
function isDemoDomain($url = null){
    return getDomain($url) == config('larapen.core.demo.domain') || in_array(getHost($url),
            (array) config('larapen.core.demo.hosts'));
}
        ...
        if (auth()->check()) {
            if (isFromAdminPanel($url)) {
                ...
                if (
                    ...
                    && !\Illuminate\Support\Str::contains(\Route::currentRouteAction(),
                        'Post\CreateOrEdit\MultiSteps\CreateController')
                    && !\Illuminate\Support\Str::contains(\Route::currentRouteAction(),
                        'Post\CreateOrEdit\MultiSteps\EditController')
                    && !\Illuminate\Support\Str::contains(\Route::currentRouteAction(),
                        'Post\CreateOrEdit\MultiSteps\PhotoController')
                    && !\Illuminate\Support\Str::contains(\Route::currentRouteAction(),
                        'Post\CreateOrEdit\MultiSteps\PaymentController')
                    && !\Illuminate\Support\Str::contains(\Route::currentRouteAction(),
                        'Post\CreateOrEdit\SingleStep\CreateController')
                    && !\Illuminate\Support\Str::contains(\Route::currentRouteAction(),
                        'Post\CreateOrEdit\SingleStep\EditController')
                    ...
                    && !\Illuminate\Support\Str::contains(\Route::currentRouteAction(),
                        'Account\ConversationsController')
...
function isFromUrlThatCanContainCountryCode(){
    if (config('settings.seo.multi_countries_urls')) {
        if (
            \Illuminate\Support\Str::contains(\Illuminate\Support\Facades\Route::currentRouteAction(),
                'SearchController')
            || \Illuminate\Support\Str::contains(\Illuminate\Support\Facades\Route::currentRouteAction(),
                'CategoryController')
            || \Illuminate\Support\Str::contains(\Illuminate\Support\Facades\Route::currentRouteAction(),
...
function createAutoLink($str,$attributes = []){
    ...
    $str = preg_replace('`([^"=\'>])((http|https|ftp)://[^\s<]+[^\s<\.)])`i',
        '$1<a rel="nofollow" href="$2"'.$attrs.' target="_blank">$2</a>',$str);
    $str = substr($str,1);
    ...
    if ($disk->exists($thumbFilePath)) {
        ...
        try {
            ...
            if (in_array($type,['logo'])) {
                ...
                $image = \Image::make($disk->get($filePath))->resize($width,$height,
                    function ($constraint) use ($ratio,$upSize){
                        if ($ratio == '1') {
                ...
                if (in_array($type,['large','big','medium','small'])) {
                    // Get image manipulation settings
                    ...
                    if ($resizeType == '0') {
                        ...
                        if ($resizeType == '1') {
                            // Fit
                            $image = \Image::make($disk->get($filePath))->fit($width,$height,
                                function ($constraint) use ($ratio,$upSize){
                                    if ($ratio == '1') {
                                        $constraint->aspectRatio();
                                    }
...
function getLoginField($value){
    ...
    if (filter_var($value,FILTER_VALIDATE_EMAIL)) {
        ...
        if (preg_match('/^((\+|00)\d{1,3})?[\s\d]+$/',$value)) {
            $field = 'phone';
        }
    }
...
function getRawBaseUrl(){
    ...
    $requestUri = (mb_substr($requestUri,0,
            strlen($laravelPublicPathLabel)) === $laravelPublicPathLabel)? '/'.$laravelPublicPathLabel :'';
...
function setBackupConfig($typeOfBackup = null){
    ...
    if ($deleteOldestBackupsWhenUsingMoreMegabytesThan > 0) {
        \Config::set('backup.cleanup.defaultStrategy.deleteOldestBackupsWhenUsingMoreMegabytesThan',
